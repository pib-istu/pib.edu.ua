...
class IceSlideShow {
	constructor(param) {
		...
		this.initialize = function(eMain, eNavigator,eNavOuter, options){
			this.setting = $extend({
				autoStart			: true,
				descStyle	    	: 'sliding',
		...
		this.navivationAnimate = function(currentIndex) {
		...
		this.setNavActive = function(index, item){
		...
		this.registerMousewheelHandler = function(element){
		...
		this.registerButtonsControl = function(eventHandler, objects, isHover){
		...
		this.start = function(isStart, obj){
		...
		this.onProcessing = function(manual, start, end){
			this.onClick = true;
			this.previousNo = this.currentNo + (this.currentNo>0 ? -1 : this.slides.length-1);
			this.nextNo 	= this.currentNo + (this.currentNo < this.slides.length-1 ? 1 : 1- this.slides.length);
			return this;
		};
		this.finishFx = function(manual){
			if(manual) this.stop();
		...
		this.getObjectDirection = function(start, end){
			return eval("({'"+this.styleMode[0]+"':["+start+", "+end+"]})");
		};
		this.fxStart = function(index, obj){

			this.fxItems[index].cancel().start(obj);
			return this;
		};
		this.jumping = function(no, manual){
			this.stop();
		...
		this.next = function(manual , item){
		...
		this.previous = function(manual, item){
			if( this.onClick ) return ;
		...
		this.play = function(delay, direction, wait){
			this.stop();
			if(!wait){ this[direction](false); }
			this.isRun = this[direction].periodical(delay,this,true);
		};
		this.stop = function(){$clear(this.isRun)}
	}
